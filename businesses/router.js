...
router.get("/search", (req, res) => {
  Business.find({
    "address.coordinates": {
      $geoWithin: {
        $centerSphere: [[req.query.long, req.query.lat], req.query.rad / 3963.2]
      }
    }
  })
    .then(categories => {
    ...
    .then(cat => {
      if (cat.length === 0) {
        ...
        Business.find({
          ...
          .sort("name")
          .then(businesses => {
            let items = businesses.filter(
              business => business.category.name === req.params.category
            );
            res.json(
              items.map(item => {
                return {
            ...
            Category.findById(req.body.category)
              ...
              .then(business => {
                return Business.create({
                  ...
                  googlePlace: req.body.googlePlace,
